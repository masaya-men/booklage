...
2. 新 text token を **既存の値をそのまま px で書いた formal version** として併設:
   ...
   --text-micro:   9px;   /* = 仮、 Phase 3 で clamp 化 */
   --text-caption: 11px;
   --text-body:    13px;
   --text-sub:     15px;
   --text-heading: 18px;
   --text-display: 22px;  /* Lightbox title 値に合わせる */
   ```
...
- 各 token を `clamp(MIN_PX, N_VW, BASE_PX)` 化 (BASE = 開発者画面 px、 N_VW = `BASE/1489×100`、 MIN = base × 0.85〜0.92):
  ```css
  --text-micro:   clamp(8px,  0.604vw, 9px);
  --text-caption: clamp(10px, 0.738vw, 11px);
  --text-body:    clamp(11px, 0.873vw, 13px);
  --text-sub:     clamp(13px, 1.007vw, 15px);
  --text-heading: clamp(15px, 1.209vw, 18px);
  --text-display: clamp(18px, 1.478vw, 22px);
  ```
- UI 要素も同形式で clamp 化:
  ```css
...
:root {
  ...
  --text-micro:   clamp(8px,  0.604vw, 9px);   /* meterCounter, fine-print */
  --text-caption: clamp(10px, 0.738vw, 11px);  /* MediaTypeIndicator, badges */
  --text-body:    clamp(11px, 0.873vw, 13px);  /* meta, body, scrollMeter labels */
  --text-sub:     clamp(13px, 1.007vw, 15px);  /* description, tweetAuthorName */
  --text-heading: clamp(15px, 1.209vw, 18px);  /* card title small */
  --text-display: clamp(18px, 1.478vw, 22px);  /* Lightbox .title */
  --text-hero:    clamp(40px, 4.835vw, 72px);  /* LP .headline (Phase 2 で要再計算) */
...
| `font-size: 13px` 直書き (sharePill 等) | **要置換** Phase 2 で `var(--text-body)` |
