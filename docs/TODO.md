...
### セッション 15 開始時の優先順位 (I-07 Phase 1 完了済)

1. **I-07 Phase 2 (Bluesky 複数画像対応)** ← **次セッション最優先**
   ...
   - Phase 1 (X) で確立した IDB photos[] 構造 + ImageCard hover + Lightbox carousel を Bluesky scraper にも適用
   - 見込み: 0.5-1 セッション
2. **B-#8 PiP click → スクロール中央寄せ**
3. **B-#1/#2/#3 サムネ系** (空白カード fallback / 再取得ボタン / 重複 URL 表示)
4. **B-#7 自由サイジング** (再挑戦するなら案 B か案 D 再チューニング)
5. **B-#12 拡大時 viewport overflow 破綻** — skyline 破綻、 max width cap か別軸対応
6. **B-#10 モバイル本格チューニング** (ユーザー希望: 後回し継続)
7. (任意) **e2e test rot 整理** — board-b0 / lightbox-flow の `DB_VERSION=9` を versionless open に修正
