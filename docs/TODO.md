...
1. **B-#8 PiP click → スクロール中央寄せ** ← **次セッション最優先** (B-#7 は保留)
2. **B-#1/#2/#3 サムネ系** (空白カード fallback / 再取得ボタン / 重複 URL 表示)
3. **B-#7 自由サイジング** (再挑戦するなら案 B か案 D 再チューニング)
4. **B-#12 (新規) 拡大時 viewport overflow 破綻** — 自由リサイズで viewport 幅を超えたカードの skyline 破綻、 max width cap か別軸対応
5. **B-#10 モバイル本格チューニング** (ユーザー希望: 後回し継続)
6. (任意) **e2e test rot 整理** — board-b0 / lightbox-flow の `DB_VERSION=9` を versionless open に修正
