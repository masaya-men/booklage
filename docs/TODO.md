...
- **ブランチ**: `master`
- **🔥 最新進捗 (2026-04-21)**: **B-embeds 完了 + 本番デプロイ済 + polish 4 round 完了** (v5 → v8)、`booklage.pages.dev` 反映済
  - **polish round で加わった機能**:
    - TweetCard は **heuristic 予測を撤廃、ResizeObserver + MutationObserver + media load event + 4 段タイマー再測定** で実 DOM 高さを測る ([components/board/cards/TweetCard.tsx](components/board/cards/TweetCard.tsx))。"Read N replies" の footer も含めた full-host 測定（article だけじゃなく全 direct children の max bottom）
    - VideoThumbCard は YouTube title が degenerate（"YouTube <id>" / URL / 空）のとき oEmbed で本物タイトルを取得 ([lib/embed/youtube-oembed.ts](lib/embed/youtube-oembed.ts))、TikTok oEmbed からも title 取得
    - TextCard は `@chenglou/pretext` で title の実測高さを計算して aspectRatio 動的化 ([lib/embed/text-card-measure.ts](lib/embed/text-card-measure.ts))。`http(s)://` prefix は表示から除去
    - 開発ツール `/seed-demos`: 実 TikTok / YouTube Shorts / Instagram URL を IDB に 1 クリック投入（ブックマーレット無しでも B-embeds 視覚確認可能）
  - **core B-embeds 機能**:
    - TweetCard / VideoThumbCard / ImageCard / TextCard 4 種 + pickCard router
    - TweetCard は react-tweet default の border/shadow/radius/actions-bar を CSS で完全 override
    - VideoThumbCard: YouTube maxres→hq→mq→0 fallback chain + TikTok oEmbed
    - TextCard: favicon + headline/editorial/index 3 モード自動切替
    - IDB schema 変更なし（既存 `cards.aspectRatio` 列を measurement writer で流用）
...
- **本番URL**: `https://booklage.pages.dev`（2026-04-21 deploy **v8**, SW `v8-2026-04-21-b-embeds-media-load-retries`）
...
- **テスト**: **139 vitest PASS / 0 FAIL** + **4 Playwright E2E PASS**。tsc strict clean

### user 実機テストで判明した残課題（次セッション対応候補）

1. **動画付きツイートの表示が一部おかしい** — 例: [heyirfanaziz/status/2044092673141944565](https://x.com/heyirfanaziz/status/2044092673141944565)。v8 の media load event + 4段タイマー再測定を入れたが、コンソールで IDB aspectRatio リセット+reload しても改善せず。original root cause 未特定（user 判断で一旦先送り、priority 低）。可能性: react-tweet 内部で video placeholder が特殊レイアウト、我々の direct-children 測定で捕まえきれない / aspectRatio 書き込みはされているが masonry が反映してない / 特定 tweet ID 固有の問題
2. **YouTube カードのタイトルバー視認性** — 暗いサムネでは白文字 + 下からのグラデ overlay でも読みづらい。user 了解済の「後でブラッシュアップ」項目
3. **ブックマーレット / 保存機能の再生** — `/save` ルートは生きてるが URL param 経由でしか保存できない状態。ブックマーレット本体 (`public/bookmarklet.js` と Board 上の drag link) は B0 リビルド時に削除されたまま。spec は [docs/superpowers/specs/2026-04-14-s5-bookmarklet.md](docs/superpowers/specs/2026-04-14-s5-bookmarklet.md) に残っている。**次セッション開始 or C 着手前に判断**
4. **YouTube タイトルが「タイトル見えないことがある」問題** — 一部の既存ブクマは oEmbed fetch が 404 / ネットワーク失敗で degenerate title のまま残留。 YouTube の video が完全削除だと oEmbed も 404 を返す

### 次セッション最初にやること

1. **`docs/TODO.md` を読む**（このファイル）
2. **user が追加で bug 報告するなら即 fix**（上記 1-3 のいずれか、user 選択）
3. **C (Multi-playback) に進むなら brainstorming から**: specs + plan 新規作成 → subagent-driven-development で実装
...
### ブックマーレット復活着手のコピペ用 prompt（別オプション）

```
docs/superpowers/specs/2026-04-14-s5-bookmarklet.md を読んで、
ブックマーレット復活の brainstorming → spec 再確認 → plan 作成 → subagent-driven-development で実装してください。
現状: public/bookmarklet.js と Board のドラッグ link は B0 リビルド時に削除済。
/save ルートと SavePopup は健在。Cloudflare Pages Function /api/ogp も使える状態。
```
