...
1. **I-07 Phase 1 (X tweet 複数画像 hover 切替) を subagent-driven で実装** ← **次セッション最優先**
   - **Plan**: [docs/superpowers/plans/2026-05-12-i07-multi-image-hover-phase1.md](./superpowers/plans/2026-05-12-i07-multi-image-hover-phase1.md) — 11 タスク (~8h)
   - **Spec**: [docs/superpowers/specs/2026-05-12-multi-image-hover-design.md](./superpowers/specs/2026-05-12-multi-image-hover-design.md)
   - **実行方法**: セッション開始直後に `superpowers:subagent-driven-development` skill を invoke、 plan ファイルを渡して 1 タスクずつ subagent dispatch
   - **完成定義**: Task 11 まで完了 = 本番に X 複数画像 hover swap + Lightbox carousel が live、 ユーザー実機 OK
2. **I-07 Phase 2 (Bluesky)** — Phase 1 完成後、 0.5-1 セッション。 spec の §4-2 + §7 Phase 2 参照
3. **B-#8 PiP click → スクロール中央寄せ**
4. **B-#1/#2/#3 サムネ系** (空白カード fallback / 再取得ボタン / 重複 URL 表示)
5. **B-#7 自由サイジング** (再挑戦するなら案 B か案 D 再チューニング)
6. **B-#12 拡大時 viewport overflow 破綻** — skyline 破綻、 max width cap か別軸対応
7. **B-#10 モバイル本格チューニング** (ユーザー希望: 後回し継続)
8. (任意) **e2e test rot 整理** — board-b0 / lightbox-flow の `DB_VERSION=9` を versionless open に修正
