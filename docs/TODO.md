...
### コード状況 (リブランド前)
- **ブランチ**: `master` 単一運用、 セッション 14 で **I-07 Phase 1 完了 + 本番 deploy 済** (2026-05-12)
- **本番**: `https://booklage.pages.dev` に **I-07 Phase 1 (X 複数画像 hover + Lightbox carousel)** まで deploy 済。 IDB v12 (photos field) / ImageCard dot indicator / Lightbox ↑↓ nav / 既存 tweet auto-backfill
...
### セッション 15 開始時の優先順位 (I-07 Phase 1 完了済 + 実機 FB あり + sizing 哲学策定済)

1. **I-07-#3 動画+画像 mix tweet 対応** ← **次セッション最優先 (実機 FB)**
   - 動画 1 + 画像 N の tweet で Lightbox 切替できない、 板上ドットが画像枚数しか出ない
   - まず brainstorming: video + photos 混合 carousel をどう見せるか
2. **I-07-#1 save 時 backfill (Lightbox open 不要化)** — 仕様再設計の brainstorming
3. **I-07-#2 hover 切替演出のリッチ化** — cross-fade / blur / zoom-pan polish 候補
4. **I-07 Phase 2 (Bluesky 複数画像対応)** — 上記実機 FB の片付け後
   - **Spec**: [docs/superpowers/specs/2026-05-12-multi-image-hover-design.md](./superpowers/specs/2026-05-12-multi-image-hover-design.md) §4-2 + §7 Phase 2 参照
5. **B-#8 PiP click → スクロール中央寄せ**
6. **B-#1/#2/#3 サムネ系** (空白カード fallback / 再取得ボタン / 重複 URL 表示)
7. **B-#7 自由サイジング** (再挑戦するなら案 B か案 D 再チューニング)
8. **B-#12 拡大時 viewport overflow 破綻** — skyline 破綻、 max width cap か別軸対応
9. **B-#10 モバイル本格チューニング** (ユーザー希望: 後回し継続)
10. (任意) **e2e test rot 整理** — board-b0 / lightbox-flow の `DB_VERSION=9` を versionless open に修正
11. **AllMarks sizing 哲学移行 (Phase 1)** — `globals.css` に `html { font-size: 16px }` + `--container-max: 1489px` + `--text-scale-multiplier: 1` 追加 (見た目変化ゼロ、 15 分作業)。 詳細 `docs/specs/2026-05-12-sizing-migration-spec.md`
12. **AllMarks sizing 哲学移行 (Phase 2-6)** — 別タスク、 rem → px 化 + clamp+vw 化 + アプリ内 text size 設定

### セッション 13 終了時の handoff (2026-05-12 深夜)

- 今日のセッションは **brainstorm + spec + plan の準備のみ**、 コード変更はゼロ
- B-#7 試みは revert 済、 working tree クリーン
- origin/master に push 済
- 明日は **完全クリーンなセッション**で開始する想定
- 開始時の最初の発言: 「I-07 Phase 1 を subagent-driven で実行して」 等で OK
