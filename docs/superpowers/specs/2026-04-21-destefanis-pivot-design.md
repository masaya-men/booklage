...
destefanis の UI/UX/トンマナを忠実に複製。AllMarks 独自の flourish（brand accent color、signature motion 等）は入れない。個性化は launch 後にユーザー反応を見てから着手。

### L2. **AllMarks にブランドカラーは未決定**
...
| 4 | 視覚スコープ | 案 X ハイブリッド（destefanis board + AllMarks sidebar identity 維持） |
...
│ AllMarks (brand) │               [Filter ▾] [表示 ▾] │ ← top-right pill
│ 🔍 検索           │                                 │
...
- **内容**:
  ...
  - 右: AllMarks logo (Playfair italic 15px) + label
