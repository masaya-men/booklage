...
2. /save popup が開く（別タブ）
3. ユーザーが [保存する] ボタン押下
4. bookmark を IDB に書き込み (tags: []、Inbox 扱い)
5. BroadcastChannel('booklage') で { type: 'bookmark-saved' } 送信
6. popup が auto-close（1.5s）
7. board タブに戻る
8. board が BroadcastChannel listener で検知 → IDB から再読み込み → 新カードが top-left に静かに fade-in
```
...
- `components/bookmarklet/SavePopup.tsx` — 既存の BroadcastChannel 送信は OK、text を「Inbox に保存しました」に更新
...
- **保存時のフォルダ選択 UI** — /save popup からフォルダ選択セクション削除。単に「Inbox に保存」ボタンのみ
...
### E2E (Playwright)
- Bookmarklet /save → BroadcastChannel → board に entrance animation で反映（現 bug 解消確認）
...
- **/save popup の未分類情報の提示**: 「このカードは Inbox に入ります」と明示する？暗黙？
