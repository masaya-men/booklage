...
AllMarks の核となるバイラル機能を実装する。ユーザーが自分のブクマコレクションから「シェア用ムードボード」を組み立て、**画像 + URL の両方**として SNS に投稿できる。受信者はブラウザで同じコラージュを閲覧でき、任意で自分のボードへ取り込める。

### Why this matters
- 「ブクマ管理ツール」と「ビジュアル表現ツール」と「バイラルシェア」の 3 点融合が AllMarks の差別化軸。Share がその拡散経路。
...
8. **Action sheet**:
   - 「画像をダウンロード」(PNG)
   - 「URL をコピー」(クリップボード)
   - 「X で投稿」(Web Intent — 画像はユーザー手動添付、URL + 「AllMarks で見る」テキスト自動)
...
| ID | 表示名 | 比率 | 用途 |
|---|---|---|---|
| `free` | 全体 | viewport ratio | ボード全体を切らずにキャプチャ。ブログ、AllMarks 自体の宣材 |
