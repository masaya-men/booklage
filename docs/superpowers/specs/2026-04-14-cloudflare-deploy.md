...
Booklage を Cloudflare Pages にデプロイし、`booklage.pages.dev` で世界中からアクセス可能にする。
...
- Booklage は 95% が静的コンテンツ → 静的配信が最速・最安定
...
| `NEXT_PUBLIC_APP_NAME` | `Booklage` | `Booklage` |
...
`next dev` 時は Pages Functions が動かないため、API 呼び出しが失敗する。ただし Booklage の主要機能（カード表示、ドラッグ、フォルダ管理等）は IndexedDB のみで動作するため、`next dev` でも大部分の開発が可能。API が必要な開発（URL ペースト → OGP 取得）のみ `npm run preview` を使う。
