...
ブクマレットの「**popup chrome (タイトル + アドレスバー) が常に出る**」体験的天井を、**Chrome 拡張機能** + **Document Picture-in-Picture API** + **content script 経由の cursor pill** で超える。同時に AllMarks 体験そのもの (= "表現ツール" としての気持ちよさ) を保存の瞬間まで持ち込む。
...
| PiP 個別カードクリック | AllMarks タブで該当カードに smooth scroll + glow (タブ無しなら新規 open) |
...
| Cursor pill 言語 | **A: 全英語** (`AllMarks · Saving / Saved / Failed`) |
...
| **右クリック「AllMarks に保存」** | ページ contextual menu 経由で保存 | — |
| **キーボードショートカット** | デフォルト `Ctrl+Shift+B` (Mac: `Cmd+Shift+B`)、`chrome://extensions/shortcuts` でカスタム可 | デフォルト固定 |
| **リンク右クリック「AllMarks にリンクを保存」** | リンク URL を引数として保存 (現ページではなくリンク先) | — |

**v0 で省く**: 画像右クリック (= image-only bookmark) は AllMarks の card データ構造に乗りにくいので保留、v1 候補。
...
| 経路 | 詳細 | 制約 |
|------|------|------|
| **A: ボード上の "Pop out" ボタン** | ボード TopHeader 右側 (existing actions エリア内) に PiP アイコンボタンを追加。click → PiP 開 | AllMarks タブ要 active 状態 |
| **B: 設定での auto-open** (デフォルト OFF) | `chrome.storage.sync.set({ autoOpenPip: true })` 時、AllMarks タブの最初の任意 click 直後に auto-launch | user gesture 制約により完全な無自動は不可、最初の 1 click は要 |
...
│              │   サムネ画像      │   │
│              └──────────────────┘   │
│                  AllMarks    [×]    │  ← 底部 12px wordmark + close
└─────────────────────────────────────┘
```
...
[0→200ms]  "AllMarks" wordmark が 28px center → 12px bottom-center に
...
- Font: Geist 13px / 600 (AllMarks)、12px / 400 (state label)
...
- **v1+**: AllMarks アプリの theme 切替時に `chrome.storage.sync.set({ activeTheme })` → 拡張 content script が読んで対応 CSS class を適用
...
├── popup.js
├── popup.css
├── pip.html               ← PiP の中身 (AllMarks stack 表示)
...
## 8. 新規 AllMarks 側エンドポイント — `/save-iframe`
...
AllMarks タブが開いてる時、PiP は そのタブの BroadcastChannel に subscribe。但し、拡張からの保存は PiP に直接通知不可 (拡張 → booklage.pages.dev の通信は postMessage 限定)。
...
| 項目 | 既定 | 説明 |
|------|------|------|
| `autoOpenPip` | `false` | AllMarks タブで最初の click 時に PiP auto-launch |
...
8. **配布パッケージ名**: `AllMarks` (シンプル、サービス名そのまま)
